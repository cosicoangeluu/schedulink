...
interface AddEventModalProps {
  onClose: () => void;
  onAdd: (event: { name: string; description: string; start_date: string; end_date?: string; venues: number[]; equipment: EquipmentItem[]; application_date: string; rental_date: string; behalf_of: string; contact_info: string; nature_of_event: string; requires_equipment?: boolean; chairs_qty?: number; tables_qty?: number; projector?: boolean; other_equipment?: string; setup_start_time?: string; setup_end_time?: string; setup_hours?: number; event_start_time?: string; event_end_time?: string; event_hours?: number; cleanup_start_time?: string; cleanup_end_time?: string; cleanup_hours?: number; total_hours?: number }) => Promise<void>;
}
...
export default function AddEventModal({ onClose, onAdd }: AddEventModalProps): React.ReactElement {
  ...
  const { formData, setFormData, availableVenues, availableEquipment, handleInputChange, handleVenueChange, handleEquipmentChange } = useEventForm({
    initialData: {},
    isEdit: false
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.name && formData.start_date) {
      ...
      if (detectedConflicts.length > 0) {
        console.log('BLOCKING submission due to conflicts');
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-red-100 text-sm">Fill in the details to schedule your event</p>
                </div>
              </div>
              <button
                onClick={onClose}
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-red-800 font-medium mb-3">
                      This event conflicts with existing approved events. You cannot create this event with the current date, time, and venue.
                    </p>
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      ...
                      Cannot Create - Conflict Detected
                    </>
                  ) : (
                    <>
