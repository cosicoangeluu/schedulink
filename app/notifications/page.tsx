...
export default function NotificationsPage() {
  ...
  const handleApprove = async (notificationId: number) => {
    try {
      const response = await fetch(`http://localhost:5000/api/notifications/${notificationId}/approve`, {
        method: 'PUT',
      });
  ...
  const handleDecline = async (notificationId: number) => {
    try {
      const response = await fetch(`http://localhost:5000/api/notifications/${notificationId}/decline`, {
        method: 'PUT',
      });
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 p-4 sm:p-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Event Approval Requests</h1>
          <p className="text-gray-600 mt-1">Review and manage event approval requests</p>
        </div>
