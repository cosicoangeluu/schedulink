...
router.post('/', async (req, res) => {
  ...
  try {
    const [result] = await pool.execute(
      ...
      [name, description, start_date, end_date || null, 'pending']
    );
    ...
    res.status(201).json({ id: eventId, name, description, start_date, end_date, status: 'pending' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const [result] = await pool.execute(
      ...
      [name, description, start_date, end_date || null, req.params.id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM registrations WHERE eventId = ?', [req.params.id]);
