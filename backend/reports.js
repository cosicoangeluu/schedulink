...
router.get('/', async (req, res) => {
  try {
    ...
    // Query attendee counts per event
    const [attendeeCounts] = await pool.execute(`
      SELECT eventId, COUNT(*) as attendees
      FROM registrations
      WHERE status = 'registered'
      GROUP BY eventId
    `);

    // Map attendee counts by eventId
    const attendeeMap = {};
    attendeeCounts.forEach(row => {
      attendeeMap[row.eventId] = row.attendees;
    });

    // Add attendees count to each report
    const reportsWithAttendees = reports.map(report => ({
      ...report,
      attendees: attendeeMap[report.eventId] || 0
    }));
